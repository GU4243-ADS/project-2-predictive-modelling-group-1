...
set.seed(2)
train=sample(1:nrow(mydata),nrow(mydata)*0.5)
test=-train
...
```
```{r}
tree.mydata = tree(labels~., data = train_data)
plot(tree.mydata)
#summary(tree.mydata)
text(tree.mydata,pretty=0)
```
```{r}
tree.pred=predict(tree.mydata,test_data,type="class")
mean(tree.pred==test_label) #0.6075
```
```{r}
library(MASS)
library(tree)
...
set.seed(1)
bag.mydata =randomForest(labels???.,data=features,
...
library (gbm)
set.seed (1)
boost.boston = gbm(labels???.,data=Boston [train ,], distribution=
